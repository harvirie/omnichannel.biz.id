...
function wa_floating_chat_render_widget() {
    ?>
    <style>
    #wa-widget{position:fixed;bottom:24px;right:24px;z-index:100000;display:flex;flex-direction:column;align-items:flex-end;font-family:ui-sans-serif,system-ui,sans-serif;}
    #wa-form-container{display:none;position:absolute;bottom:80px;right:0;background:white;border-radius:16px;box-shadow:0 20px 25px -5px rgba(0,0,0,.15);border:1px solid #e5e7eb;width:320px;max-height:calc(100vh - 120px);flex-direction:column;overflow:hidden;opacity:0;transition:opacity .3s ease;}
    #wa-form-container.is-open{display:flex;}
    #wa-toggle-btn{background:#25D366;color:white;border-radius:9999px;width:48px;height:48px;border:none;box-shadow:0 4px 14px rgba(0,0,0,.12);cursor:pointer;display:flex;align-items:center;justify-content:center;flex-shrink:0;transition:transform .15s cubic-bezier(0.4, 0, 0.2, 1);touch-action:manipulation;-webkit-tap-highlight-color:transparent;}
    #wa-toggle-btn:hover{transform:translateY(-2px) scale(1.05);}
    #wa-toggle-btn:active{transform:scale(0.85) !important;}
    @media(max-width:767px){
      #wa-widget{bottom:20px;right:20px;}
      #wa-form-container{width:calc(100vw - 48px);bottom:74px;right:0;max-height:calc(100vh - 150px);}
    }
    </style>
    <div id="wa-widget">
      ...
      <button id="wa-toggle-btn" aria-label="Hubungi via WhatsApp">
    ...
    function initWAWidget() {
        ...
        // Use touchend + preventDefault to prevent ghost-click that triggers keyboard
        let waTouchMoved = false;
        toggle.addEventListener('touchstart', () => { waTouchMoved = false; }, { passive: true });
        toggle.addEventListener('touchmove',  () => { waTouchMoved = true;  }, { passive: true });
        toggle.addEventListener('touchend', (e) => {
            if (waTouchMoved) return;
            e.preventDefault();
    ...
    // Initialize on DOM ready — avoids flash/delay on mobile
